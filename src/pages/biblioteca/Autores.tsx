...
import { useAutores, useCreateAutor, useUpdateAutor, useDeleteAutor } from "@/hooks/useBiblioteca";

interface FormState { nome: string; descricao: string; }
const emptyForm: FormState = { nome: "", descricao: "" };

const Autores = () => {
  const { data: autores = [], isLoading } = useAutores();
  const createMut = useCreateAutor();
  const updateMut = useUpdateAutor();
  const deleteMut = useDeleteAutor();
  ...
  const filtered = autores.filter((a) => a.nome.toLowerCase().includes(search.toLowerCase()));
  ...
  return (
    <div>
      <PageHeader title="Autores" subtitle="Gestão de autores do acervo">
        {canEdit && (
      ...
      <div className="relative max-w-sm mb-6">
        ...
        <Input placeholder="Buscar autor..." value={search} onChange={(e) => setSearch(e.target.value)} className="pl-10 bg-card border-border" />
      </div>
            ...
            <tbody>
              {filtered.map((a) => (
                <tr key={a.id} className="border-b border-border/50 hover:bg-muted/20 transition-colors">
                  ...
                  <td className="px-5 py-3 text-sm text-muted-foreground">{a.descricao || "—"}</td>
                  {canEdit && (
                    <td className="px-3 py-3">
                      <div className="flex items-center justify-center gap-1">
                        <Button variant="ghost" size="icon" className="h-7 w-7 text-muted-foreground hover:text-primary" onClick={() => { setEditingId(a.id); setForm({ nome: a.nome, descricao: a.descricao || "" }); setDialogOpen(true); }}>
              ...
              {filtered.length === 0 && <tr><td colSpan={3} className="text-center py-8 text-sm text-muted-foreground">Nenhum autor encontrado.</td></tr>}
            </tbody>
          </table>
        </div>
      </div>
      ...
      <Dialog open={dialogOpen} onOpenChange={setDialogOpen}>
        <DialogContent className="bg-card border-border">
          <DialogHeader><DialogTitle className="text-card-foreground">{editingId ? "Editar Autor" : "Novo Autor"}</DialogTitle></DialogHeader>
