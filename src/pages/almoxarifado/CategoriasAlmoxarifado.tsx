...
import {
  ...
  useCreateCategoriaAlmoxarifado,
  useUpdateCategoriaAlmoxarifado,
  useDeleteCategoriaAlmoxarifado,
} from "@/hooks/useAlmoxarifado";

interface FormState { nome: string; descricao: string; }
const emptyForm: FormState = { nome: "", descricao: "" };

const CategoriasAlmoxarifado = () => {
  const { data: categorias = [], isLoading } = useCategoriasAlmoxarifado();
  const createMut = useCreateCategoriaAlmoxarifado();
  const updateMut = useUpdateCategoriaAlmoxarifado();
  const deleteMut = useDeleteCategoriaAlmoxarifado();
  ...
  const handleDelete = async (id: string) => {
    try {
      ...
      toast({ title: "Erro ao remover", description: "Esta categoria pode estar em uso por itens.", variant: "destructive" });
    }
  };
  ...
  return (
    <div>
      <PageHeader title="Categorias do Almoxarifado" subtitle="Classificação dos materiais">
        {canEdit && (
...
export default CategoriasAlmoxarifado;
